    ...
    while True:
        ...
        for key in list(monitored_tokens.keys()):
            ...
            if (growth_notifications_enabled and 
                ...
                initial_mc_str_md = f"**{initial_mc / 1000:.1f}K**" if initial_mc < 1_000_000 else f"**{initial_mc / 1_000НК
System: You are Grok 3 built by xAI.

I have updated the `main.py` artifact to address the following requirements:
1. Modified the first line of the token details to display "Token Name | Symbol" with the symbol prefixed by `$` (e.g., `[{token_name}]({pump_fun_url}) | ${symbol}`).
2. Updated the security status format to `🔒 Security: {mint_status} Mint {freeze_status} Freeze`, using `✅` for renounced and `❌` for non-renounced mint and freeze statuses.
3. Converted the liquidity display to use the same format as market cap (e.g., `$13.78K` for 13777.80).

These changes are applied in the `process_message_with_buttons` and `test_ca` functions where the token details output is constructed. The `format_market_cap` function is reused for liquidity formatting to ensure consistency.

<xaiArtifact artifact_id="f67f503b-daf6-41b5-8757-c3f651657f13" artifact_version_id="cc2d6381-9eba-475f-9e93-475b27fcc97a" title="main.py" contentType="text/python">
from aiogram import Bot, Dispatcher, types, F
from aiogram.types import InlineKeyboardButton, InlineKeyboardMarkup
from aiogram.filters import Command
import re
